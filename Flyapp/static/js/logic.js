let url = 'https://testpy.fly.dev/api';
// let url = '/api';
...
d3.json(url).then((data) => {
    ...
    let rebuilt = showRebuiltCheckbox.checked;


    // Initiate functions
    geninfo(make, model, province, minYear, maxYear, rebuilt);
    avg_prov(make, model, province, minYear, maxYear, rebuilt);
    avg_nat(make, model, minYear, maxYear, rebuilt);
    avg_miles_prov(make, model, province, minYear, maxYear, rebuilt)
    avg_miles_nat(make, model, minYear, maxYear, rebuilt)
    aval_pie_prov(make, model, province, minYear, maxYear, rebuilt)
    aval_pie_nat(make, model, minYear, maxYear, rebuilt)
    in_prov(make, model, province, minYear, maxYear, rebuilt)
    in_nat(make, model, minYear, maxYear, rebuilt)
    insights(make, model, province, minYear, maxYear, rebuilt)
    showMap()
        
});

// Search Data Panel
function geninfo(make, model, province, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
      const genInfoDiv = d3.select('#gen_info');
      ...
      let filteredData = data.listings.filter((listing) => {
        ...
        return (
          ...
          (rebuilt ? true : listing.Rebuilt_Predicted === 0)
        );
      });
...
function showMap() {
    d3.json(url).then((data) => {
      ...
      // An array containing each province's name, location, and population
      let provinces = [
        {
          location: [45.048704, -63.373213],
          name: "NS",
          results: provinceCounts.NS
        },
        {
          location: [46.582131, -66.237527],
          name: "NB",
          results: provinceCounts.NB
        },
        {
          location: [46.356024, -63.393342],
          name: "PEI",
          results: provinceCounts.PE
        },
        {
          location: [45.588011, -73.590681],
          name: "QC",
          results: provinceCounts.QC
        },
        {
          location: [43.949046, -79.041493],
          name: "ON",
          results: provinceCounts.ON
        },
        {
          location: [49.747196, -97.210134],
          name: "MB",
          results: provinceCounts.MB
        },
        {
          location: [49.8, -105.819764],
          name: "SK",
          results: provinceCounts.SK
        },
        {
          location: [49.8, -112.644366],
          name: "AB",
          results: provinceCounts.AB
        },
        {
          location: [49.344037, -122.211540],
          name: "BC",
          results: provinceCounts.BC
        }
      ];
  
      // Looping through the provinces array, create one marker for each province, bind a popup containing its name and results count, and add it to the map.
      for (let i = 0; i < provinces.length; i++) {
        let province = provinces[i];
        try {
          let marker = L.marker(province.location)
            .bindPopup(`<h3>${province.name}</h3> <hr> <h4>Results: ${province.results.toLocaleString()}</h4>`);
...
// Insights
function insights(make, model, province, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
...
function in_prov(make, model, province, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
  ...
  function in_nat(make, model, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
function avg_prov(make, model, province, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
function avg_nat(make, model, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
function avg_miles_prov(make, model, province, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
function avg_miles_nat(make, model, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
function aval_pie_prov(make, model, province, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
function aval_pie_nat(make, model, minYear, maxYear, rebuilt) {
    d3.json(url).then((data) => {
      console.log(data);
...
const showRebuiltCheckbox = document.getElementById('showRebuiltCheckbox');
...
showRebuiltCheckbox.addEventListener('change', sendOptions);
...
function sendOptions() {
  ...
  let showRebuiltValue = showRebuiltCheckbox.checked;
...
// Ensure minYear is not higher than maxYear
  ...
  geninfo(makeValue, modelValue, provValue, minYearValue, maxYearValue, showRebuiltValue);
  avg_prov(makeValue, modelValue, provValue, minYearValue, maxYearValue, showRebuiltValue);
  avg_nat(makeValue, modelValue, minYearValue, maxYearValue, showRebuiltValue)
  avg_miles_prov(makeValue, modelValue, provValue, minYearValue, maxYearValue, showRebuiltValue)
  avg_miles_nat(makeValue, modelValue, minYearValue, maxYearValue, showRebuiltValue)
  aval_pie_prov(makeValue, modelValue, provValue, minYearValue, maxYearValue, showRebuiltValue)
  aval_pie_nat(makeValue, modelValue, minYearValue, maxYearValue, showRebuiltValue)
  in_prov(makeValue, modelValue, provValue, minYearValue, maxYearValue, showRebuiltValue)
  in_nat(makeValue, modelValue, minYearValue, maxYearValue, showRebuiltValue)
  insights(makeValue, modelValue, provValue, minYearValue, maxYearValue, showRebuiltValue)
  showMap()
}